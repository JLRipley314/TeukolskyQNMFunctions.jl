...
function radial_discretized_eqn(
    ...
    A = (
        tomyC(2 * im) * om * I - tomyF(2) * (tomyF(1) + s) * X1 +
        tomyF(2) *
        (im * om * ((a^2) - tomyF(8) * (bhm^2)) + im * m * a + (s + tomyF(3)) * bhm) *
        X2 +
    ...
    B = (
        ...
        (
            ...
            (
                tomyF(4) * m * a * bhm - tomyF(4) * im * (s + tomyF(2)) * (bhm^2) +
                im * (a^2)
            ) * om +
            im * m * a +
            (s + tomyF(1)) * bhm
        ...
        X1 +
        tomyF(2) *
        (tomyF(8) * (bhm^2) * (om^2) + tomyF(6) * im * bhm * om - tomyF(1)) *
        (a^2) *
        X2
    )

    return (-(X2 - tomyF(2) * bhm * X3 + (a^2) * X4)*D2+A*D1+B):ApproxFun.space(X1)
end
...
"""
function radial_discretized_eqn_p(np::myI, s::myI, m::myI, a::myF, bhm::myF, om::myC)
    X = Vector{myI}()
    Y = Vector{myI}()
    V = Vector{myC}()
    ...
    B2 =
        tomyF(2) * (tomyF(8) * ((bhm * om)^2) + tomyF(6) * im * om * bhm - tomyF(1)) * (a^2)
...
"""
function eig_vals_vecs_c(nr::myI, s::myI, m::myI, a::myF, om::myC)
    bhm = tomyF(1) ## always have unit black hole mass
    rmin = tomyF(0)
    rmax = tomyF(
        abs(a) > 0 ? (bhm / (a^2)) * (tomyF(1) - sqrt(tomyF(1) - ((a / bhm)^2))) :
        tomyF(0.5) / bhm,
    )
